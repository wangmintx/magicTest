...
//
	...
	{ 
		...
		if (iebrowser_version >= 5.5) {
			...
			document.writeln('<table id="richEditTable" border="0" width="' + width + '" class="tablebg"><tr><td width=' + width + ' valign="top">');			
			...
			document.writeln('<iframe id="myEditor" width="' + width + '" height="' + height + '" src="'+subdirectory+'newFrame.html"></iframe>');
			...
			document.writeln('<textarea wrap="hard" id="myEditor" cols="' + parseInt(width/8) + '" rows="' + parseInt(height/15) + '">*** Your browser must be IE5.0 or above to display the editor\'s controls ***</textarea>');		
		}		
	}
	...
	function generateRichEdit(textareaName){
		ak_wysiwyg_generator(AK_width, AK_height, textareaName, AK_DisplayedButtons);
	}
	function useRichEdit(){
		var url = window.location.href;
		if(url.indexOf("?")>=0) url=url+"&useRichEdit=true";
		else url=url+"?useRichEdit=true";
		location=url;
	}
	function useRichEdit1(url){
		location=url;
	}